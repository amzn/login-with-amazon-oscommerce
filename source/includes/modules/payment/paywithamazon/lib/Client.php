...
class OffAmazonPaymentsService_Client
{
    ...
    if($this->_config['response_log']) {
        try {
            $log_req_string = print_r($parameters, true);
            $log_resp_string = print_r($response, true);
            $log_string = "\n\n\nRequest:\n" . $log_req_string . "\n\nResponse:\n" . $log_resp_string . "\n";
            file_put_contents($this->_config['response_log'], $log_string, FILE_APPEND | LOCK_EX);
        } catch(Exception $ex) {
            trigger_error('Amazon response logging failed with message: ' . $ex->getMessage(), E_USER_WARNING);
        }
    }
